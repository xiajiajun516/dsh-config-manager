...
- 新增三个切片：
  - `SyncStoreSlice`：通道表单 / 同步模式与分区勾选 / 加密与密钥开关 / 历史快照选中 / push·pull 报告 / **一键同步差异确认会话** / 回滚入口 / 错误文本；含仅内存的敏感字段（token / webdav 密码 / 加密与解密密码）；
...
- 切片提取纯函数 `toSyncStoreSlice` / `toMarketStoreSlice` / `toSnapshotsStoreSlice`（结构兼容，组件状态可直接传入）——枚举字段，保证 github 流程态、loading/busy 等瞬态不进切片；
- **白名单**：`toPersistedState()` 解构剔除同步凭据（`token/webdavPassword/encryptPassword/encryptPasswordConfirm/decryptPassword`），`applyPersisted()` 强制归零——刷新后清空、要求重输（与 Export/Import 的密码语义一致）；
- 版本兼容：`v1` 载荷缺 `panel/sync/market/snapshots` 字段时回退默认切片，不破坏旧 sessionStorage 数据。

### 视图接入（镜像模式，最小 diff）

三个视图保持 `useState` 为渲染源，新增三层接线：

1. **挂载恢复**：`useState(initFromStore)` —— 从 `runStore.getSnapshot()` 的切片重建初始状态（敏感字段：切 tab 保留内存值；刷新后已被清空）；
2. **状态镜像**：`useEffect(..., [state])` → `runStore.patch({ sync|market|snapshots: toXxxSlice(state) })` —— 模块级单例保证切 tab 不丢，sessionStorage 保证刷新恢复；
3. **卸载 flush**：卸载清理 effect 最后再镜像一次（防止「最后一次改动后立即切 tab」时镜像 effect 尚未 flush）。
...
- sessionStorage 唯一写入路径 `toPersistedState()` 白名单剔除所有敏感字段，新增敏感字段不显式放行即自动不落盘；
- 不新增任何敏感字段进入切片（市场/快照无 secret 输入；同步报告与会话 items 均已是引擎脱敏后的展示文本）。

## 测试

`src/client/run-store.test.ts` 新增三组用例：

- 同步凭据（token / webdav / 加密 / 解密密码）绝不写入 sessionStorage（白名单键级断言）；
- 同步/市场/快照切片 + 当前面板刷新往返恢复（敏感字段清空，confirmSession / market detail / restore plan·report 恢复）；
- 旧版 v1 载荷（无新字段）兼容解析 → 默认切片。

## 验证

```bash
npm run typecheck   # tsc --noEmit
npm test            # 759 用例全过（含新增）
