...
- **Profiles（配置 Profile）**：保存当前配置为 Profile / 切换 / 复制 / 重命名 / 导出 / 导入 / 删除；切换带 Preview + 快照 + 回滚（Phase 6，`src/profiles/`）。
...
测试框架为 **node:test（Node 内置，零依赖）**，沿用 m3 的既有选择（不引入 vitest）。测试位于 `src/**/*.test.ts` 与 `tests/**/*.test.ts`。
