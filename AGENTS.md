...
| 迁移前咨询（Phase 7） | `src/client/consult/ConsultCard.tsx`（+ `src/ui/migration-consult-view.ts` 纯渲染模型、`src/core/migration-consult.ts` 纯函数健康评分、`src/core/consult-source.ts` 源读取）；接线到导入向导预览 / 快照恢复计划弹窗 / 配置档案切换预览 / 一键同步差异确认 |
