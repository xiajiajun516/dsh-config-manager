...
export const zh = {
  ...
  'snapshots.hint': '导出备份 = 便携 ZIP（下载 / 一键导入 / 删除，含定时全量备份）；快照恢复 = 恢复到导入前状态：整文件还原 settings/patch、卸载导入期间新增插件、补偿被修改文件，执行前会把当前文件备份到快照的 pre-restore 目录。',
