...
export const zh = {
  ...
  'overview.quick.backupSkippedOther': '已跳过：备份服务当前无法执行（详情见宿主日志）',
  ...
  'picker.sessionWorkspaceLinked': '会话与工作区联动勾选：勾上会话会自动勾上它所属的工作区；取消工作区会取消它的会话',
  ...
  'import.preview.rebase': '已自动重定基（备份来自另一台机器的基础路径）：{from} → {to}；这一步不需要手工映射',
  ...
  // 日志表头计数（把 500 行流水压成「成功/跳过/警告/失败」四个数）
  'import.log.counts': '成功 {ok} · 跳过 {skip} · 警告 {warn} · 失败 {fail}',
  'import.log.filterProblems': '只看问题',
  'import.log.filterAll': '显示全部',
  'import.log.problemsEmpty': '本次没有失败或警告的项',
  // 结果页固定操作栏的收尾动作（替代原先埋在报告卡底部、被布局裁掉的「完成」）
  'import.done': '完成',
  ...
  // 路径来源徽章：此前直接渲染 core 的枚举字面量（kind：missing / platformMismatch），
  // 用户读成「源路径不存在」——missing 的真实语义是「需为本机指定新位置」（core 不做存在性探测）。
  'import.paths.kind.missing': '需指定新位置',
  'import.paths.kind.platformMismatch': '跨平台路径',
  'import.paths.kind.homeMismatch': '基础路径不同',
  ...
  // —— 运行中心（活动抽屉「进行中」段；终止 = 协作式取消）——
  'shell.drawer.runs': '运行',
  'shell.status.runningAction': '查看进行中的任务',
  'runs.loading': '正在读取任务列表…',
  'runs.loadFailed': '读取任务列表失败：{message}',
  'runs.empty': '近期没有任何任务（运行中的任务会实时出现在这里）',
  'runs.retentionHint': '刚结束的任务保留 30 分钟；完整操作审计见「迁移历史」',
  'runs.runningBadge': '{count} 个进行中',
  'runs.awaitingBadge': '{count} 个等待你选择',
  'runs.awaitingDecision': '等待你选择',
  'runs.decideNow': '去选择',
  'runs.cancelAction': '终止…',
  'runs.counts': '成功 {ok} · 警告 {warn} · 失败 {fail} · 跳过 {skip}',
  'runs.kind.export': '导出',
  'runs.kind.import': '导入',
  'runs.kind.autosync': '自动同步',
  'runs.kind.sync-apply': '一键同步',
  'runs.kind.backup-schedule': '定时备份',
  'runs.kind.restore': '快照恢复',
  'runs.kind.profile-switch': '档案切换',
  'runs.kind.recovery': '事故恢复',
  'runs.status.running': '进行中',
  'runs.status.done': '已完成',
  'runs.status.failed': '失败',
  'runs.progress.running': '执行中',
  'runs.progress.done': '已完成',
  'runs.progress.failed': '已失败',
  'runs.cancelTitle': '终止这个任务',
  'runs.cancelCost': '已应用 {applied} 项、未执行 {pending} 项',
  'runs.cancelCostUnknown': '已应用项数未知（宿主未上报进度计数）',
  'runs.cancel.rollback': '回滚到导入前',
  'runs.cancel.rollbackDesc': '撤销本次导入已应用的全部改动，回到导入前的快照状态',
  'runs.cancel.keep': '保留已应用项',
  'runs.cancel.keepDesc': '只停掉未执行的部分，已经写进配置的内容留在本机；随后会自动做一次启动安全审计',
  'runs.cancelKeepWarn': '「保留」会留下一笔未完成的导入：审计只能压低 DSH 启动失败的概率，不能保证。若 DSH 起不来，用「事故恢复 / SAFE MODE」或离线 CLI 处理。',
  'runs.recommended': '推荐',
  'runs.cancelAbort': '继续运行',
  'runs.cancelConfirm': '确认终止',
  'runs.cancelRequestedToast': '已请求终止：将在当前计划项完成后暂停',
  'runs.cancelWaiting': '已请求终止：等当前计划项结束后暂停（已等待 {minutes} 分钟）',
  'runs.cancelWaitingJustNow': '已请求终止：等当前计划项结束后暂停',
  'runs.cancelStuck': '长时间没有到达安全点。若出现「跳过当前插件」请先点它；仍不行请重启 DSH —— 环境锁由本进程持有，重启即释放，不重启时其他写操作会一直被拒（423）。',
  'runs.lock.title': '环境锁',
  'runs.lock.needsAction': '需要处理',
  'runs.lock.held': '使用中',
  'runs.lock.attentionDesc': '检测到上次异常退出残留的配置锁（持有进程已不存在），或锁状态无法判定：重试/重启都不会自愈，必须显式回收。',
  'runs.lock.heldDesc': '环境锁正被另一项任务持有（可能就是上面那张正在跑的卡片）：它会自行释放；若它正是那张「长时间等不到安全点」的导入，只能重启 DSH 释放。',
  'runs.lock.recover': '回收残留锁',
  'runs.lock.recoverOk': '已回收残留锁',
  'runs.lock.recoverRefused': '未回收（{state}）：只有「持有者已确证死亡」的残留锁才允许回收，活锁会自行释放',
  'runs.lock.recoverFailed': '回收失败：{message}',
  'runs.lock.readFailed': '读不到环境锁状态：{message}',
  'runs.skipRequestedToast': '已请求跳过当前插件',
  'runs.decisionAcceptedToast': '已提交你的选择',
  'runs.decisionTooLateToast': '来不及了：等待超时已按安全侧默认（回滚）继续',
  'runs.actionFailed': '操作失败：{message}',
  'runs.unsupported': '该任务不支持手动终止',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'overview.quick.backupSkippedOther': 'Skipped: the backup service cannot run right now (see host logs for details)',
  ...
  'picker.sessionWorkspaceLinked': 'Session and workspace selection are linked: checking a session also checks the workspace that owns it; unchecking a workspace unchecks its sessions',
  ...
  'import.preview.rebase': 'Base path rebased automatically (the backup came from a machine with a different DSH base path): {from} -> {to}; no manual mapping needed',
  ...
  'import.log.counts': '{ok} ok · {skip} skipped · {warn} warned · {fail} failed',
  'import.log.filterProblems': 'Problems only',
  'import.log.filterAll': 'Show all',
  'import.log.problemsEmpty': 'No failed or warned items in this import',
  'import.done': 'Done',
  ...
  'import.paths.kind.missing': 'Set a new location',
  'import.paths.kind.platformMismatch': 'Cross-platform path',
  'import.paths.kind.homeMismatch': 'Different base path',
  ...
  // —— Run center (the drawer's "In progress" segment; terminate = cooperative cancel) ——
  'shell.drawer.runs': 'Runs',
  'shell.status.runningAction': 'View running tasks',
  'runs.loading': 'Loading tasks...',
  'runs.loadFailed': 'Failed to load tasks: {message}',
  'runs.empty': 'No recent tasks (running ones appear here live)',
  'runs.retentionHint': 'Recently finished tasks are kept for 30 minutes; the full audit lives in Migration History',
  'runs.runningBadge': '{count} running',
  'runs.awaitingBadge': '{count} waiting for you',
  'runs.awaitingDecision': 'Waiting for you',
  'runs.decideNow': 'Decide',
  'runs.cancelAction': 'Terminate...',
  'runs.counts': 'OK {ok} · warn {warn} · fail {fail} · skip {skip}',
  'runs.kind.export': 'Export',
  'runs.kind.import': 'Import',
  'runs.kind.autosync': 'Auto sync',
  'runs.kind.sync-apply': 'One-click sync',
  'runs.kind.backup-schedule': 'Scheduled backup',
  'runs.kind.restore': 'Snapshot restore',
  'runs.kind.profile-switch': 'Profile switch',
  'runs.kind.recovery': 'Incident recovery',
  'runs.status.running': 'Running',
  'runs.status.done': 'Done',
  'runs.status.failed': 'Failed',
  'runs.progress.running': 'Running',
  'runs.progress.done': 'Done',
  'runs.progress.failed': 'Failed',
  'runs.cancelTitle': 'Terminate this task',
  'runs.cancelCost': '{applied} applied, {pending} not executed',
  'runs.cancelCostUnknown': 'Applied item count is unknown (the host did not report progress counts)',
  'runs.cancel.rollback': 'Roll back to before the import',
  'runs.cancel.rollbackDesc': 'Undo every change this import applied and return to the pre-import snapshot',
  'runs.cancel.keep': 'Keep applied items',
  'runs.cancel.keepDesc': 'Stop only the remaining work and keep what is already written; a launch safety audit runs right after',
  'runs.cancelKeepWarn': 'Keeping leaves an unfinished import behind: the audit only lowers the chance of DSH failing to start, it cannot guarantee it. If DSH will not start, use incident recovery / SAFE MODE or the offline CLI.',
  'runs.recommended': 'Recommended',
  'runs.cancelAbort': 'Keep running',
  'runs.cancelConfirm': 'Terminate',
  'runs.cancelRequestedToast': 'Termination requested: it pauses after the current plan item finishes',
  'runs.cancelWaiting': 'Termination requested: pauses after the current plan item ({minutes} min so far)',
  'runs.cancelWaitingJustNow': 'Termination requested: pauses after the current plan item',
  'runs.cancelStuck': 'No safe point reached for a long time. If "Skip current plugin" appears, click it first; otherwise restart DSH -- the environment lock is held by this process, so a restart releases it. Without a restart, every other write keeps being rejected (423).',
  'runs.lock.title': 'Environment lock',
  'runs.lock.needsAction': 'Action needed',
  'runs.lock.held': 'In use',
  'runs.lock.attentionDesc': 'A leftover config lock from an abnormal exit (its holder no longer exists), or the lock state cannot be determined: retrying or restarting will NOT fix it -- recover it explicitly.',
  'runs.lock.heldDesc': 'The environment lock is held by another task (likely the running card above). It releases itself; if it is that import stuck without reaching a safe point, only restarting DSH can release it.',
  'runs.lock.recover': 'Recover leftover lock',
  'runs.lock.recoverOk': 'Leftover lock recovered',
  'runs.lock.recoverRefused': 'Not recovered ({state}): only a leftover lock whose holder is proven dead may be recovered; a live lock releases itself',
  'runs.lock.recoverFailed': 'Recovery failed: {message}',
  'runs.lock.readFailed': 'Cannot read the environment lock state: {message}',
  'runs.skipRequestedToast': 'Requested to skip the current plugin',
  'runs.decisionAcceptedToast': 'Your choice was submitted',
  'runs.decisionTooLateToast': 'Too late: the wait timed out and the safe-side default (roll back) already continued',
  'runs.actionFailed': 'Action failed: {message}',
  'runs.unsupported': 'This task does not support manual termination',
