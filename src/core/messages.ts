...
export const zh = {
  ...
  'export.sessionsWorkspacesCoupled': '已连带导出 {count} 条与会话相关的工作区记录（会话要在目标机显示出来，必须有工作区指向它的 cwd）',
  'export.sessionsDeclaredInWorkspaces': '已把本次导出的 {count} 次会话声明进 {workspaces} 条工作区记录（DSH 工作区 sessionIds 覆盖率很低，漏了这一步目标机就看不见这些对话）',
  'export.sessionsWorkspacesCarriedAll': '选中的会话没有登记在任何工作区记录里：已连带导出全部 {count} 条工作区记录（漏带工作区 = 目标机上这些对话看不见）',
  'export.sessionsWithoutWorkspaces': '本机没有任何工作区记录，会话只能单独导出：目标机上这些对话可能不会出现在工作区里（会话可见的前提是有一条工作区指向它的 cwd）',
  'export.sessionsWorkspacesUnreadable': '读不到本机工作区记录，无法确认会话归属：会话已单独导出，目标机上可能显示不出来',
  ...
  'import.sessionsWithoutWorkspaces': '该备份包含会话但没有任何工作区记录：导入后这些对话可能不会出现在 DSH 的工作区列表里（建议在源机用新版插件重新导出 —— 导出会话会自动连带其所属工作区）',
  'import.subagentSessionsWithoutParents': '本包含 {count} 个子代理会话，但它们的父对话不在包内（{units}）：DSH 工作区只在父对话之下显示子代理会话，这些会话导入后不会出现在工作区列表里（请在源机重新导出时连同父对话一起勾选）',
  'export.sessionParentsCoupled': '已连带导出 {count} 个父对话（DSH 工作区只在父对话之下显示子代理会话；不带父对话时导入后在工作区里看不到）',
  'export.sessionParentsUncoupled': '有 {count} 个子代理会话未能连带其父对话：{detail} —— 只带子会话时，导入后 DSH 工作区里不会显示它们',
  'export.sessionChildrenCoupled': '已连带导出 {count} 个子代理会话（DSH 把子对话显示在父对话的树状图下面；只带父对话时那棵树是空的）',
  'export.sessionRelationNotFound': '本机找不到这条相关会话',
  'export.sessionParentTooLarge': '相关会话超出本分区大小上限',
  ...
  'import.secretAlreadyConfigured': '凭据 {ref} 本机已配置，跳过补录（保留本机值）',
  'import.secretFromArchive': '凭据 {ref} 随加密备份恢复（导入时写回本机）',
  ...
  'import.sessionRelocated': '会话 {unit} 的目录与首帧 cwd 不一致，已自动归位：{from} → {to}（只移动目录，不改内容）',
  'import.sessionRelocateFailed': '会话 {unit} 的目录与首帧 cwd 不一致，且未自动归位（{reason}）；若放着不管，下次启动 DSH 会报 corrupt session log —— 请在导入向导里补上路径映射（或事后用离线修复工具 sessions repair 处理）',
  'import.sessionRewritten': '会话 {unit}：首帧 cwd 已按路径映射改写 {from} → {to}，并把会话目录归位到 {key}',
  'import.sessionRewriteFailed': '会话 {unit}：首帧 cwd 改写失败（{reason}），会话文件与会话目录保持原样',
  'import.sessionRewriteRolledBack': '会话 {unit}：首帧已改写但目录未能归位（{reason}），已把首帧改回原值（避免下次启动报 corrupt session log）',
  'import.sessionRewriteRollbackFailed': '会话 {unit}：首帧已改写、目录未能归位（{reason}），且回滚失败 —— 请手动检查该会话目录（否则下次启动 DSH 会报 corrupt session log）',
  ...
  'runs.cancelUnsupported': '该任务不支持手动终止（目前只有导入支持在计划项边界协作式取消）',
  'runs.cancelWaitingSafePoint': '已请求终止：等当前计划项结束后暂停（已同时中止当前项的子进程，若有）',
  'import.cancelRequested': '收到终止请求，正在安全点（计划项边界）暂停…',
  'import.cancelAwaitingDecision': '已暂停：等待你选择「回滚」还是「保留已应用项」',
  'import.cancelDecisionTimeout': '等待选择超时，已按安全侧默认处理（回滚）',
  'import.cancelRollingBack': '已按你的选择回滚本次导入（已应用 {count} 项）',
  'import.cancelKept': '已终止导入并保留已应用项：{applied} 项已应用、{skipped} 项未执行',
  'import.cancelKeptSnapshotKept': '本次导入未完成：导入前快照 {snapshotId} 仍然保留，可稍后在「备份」页手动回滚这一笔',
  'import.cancelKeptSkippedItem': '导入已被用户终止（选择保留已应用项）：本项未执行',
  'import.cancelKeptJournalFailed': '记录「{id} 未执行」失败（{reason}）：事务日志与盘面可能不一致，请以盘面为准',
  'import.cancelKeptUnchecked': '启动自洽审计有未执行的检查（{items}）：这些方面未经证实',
  'import.cancelKeptNoAudit': '启动自洽审计未执行（{reason}）：保留后的启动安全**未经验证**，DSH 起不来时请用事故恢复 / 离线 CLI',
  'bootSafety.bundlesPruned': '插件清单里 {count} 个包解析不到（插件没装成或已被删）：已从 dsh.profile.bundles 剔除 {names}——不剔除会让 DSH 下次启动直接失败',
  'bootSafety.bundlesPrunedFailed': '插件清单里有 {count} 个包解析不到（{names}），但写回 profile package.json 失败：{reason}——请手动剔除后再启动 DSH',
  'bootSafety.bundleListUnreadable': '读不到 profile 插件清单 {rel}（{reason}）：无法确认启动安全的插件集合',
  'bootSafety.criticalFileUnreadable': '启动关键文件 {rel} 读取失败：{reason}',
  'bootSafety.criticalFileUnparsable': '启动关键文件无法解析（{rel}）：{reason}',
  'bootSafety.patchFileMissing': 'pnpm-workspace.yaml 的 patchedDependencies 指向的 patch 文件不存在：{ref}（缺失会让 pnpm 拒绝一切 add）',
  'bootSafety.extraCheckFailed': '附加启动检查失败：{reason}',
  ...
  'host.workspaceMissingTarget': '目标机不存在工作区 {id}（无法登记会话）',
  'host.fsPathNotAbsolute': '拒绝在建目录时使用相对路径（会跟随进程工作目录）: {path}',
  'host.fsPathDotDot': '拒绝在建目录时使用含 .. 的路径（来源可疑）: {path}',
  'host.fsPathNotDirectory': '路径上已存在同名文件（不是目录）: {path}',
  ...
  'adapter.fileSectionByteLimit': '{type} 分区累计体积达到单分区上限 {limit}（实际保留 {kept}）：{count} 个单元整块未进备份（单元不可拆分，宁可少带不可半带）—— {detail}',
  ...
  'adapter.workspaceDirCreated': '已创建备份里声明、但本机缺失的工作区目录：{dirs}',
  'adapter.workspaceDirFailed': '创建目录失败（退回非致命警告）：{path} — {msg}',
  'adapter.workspaceDirUnsafe': '路径不是可安全创建的绝对路径（相对路径或含 ..），未建目录：{path}',
  'adapter.workspaceSessionsAttached': '已把 {count} 个会话登记进该工作区',
  'adapter.workspaceSessionsPending': '{ok} 个会话已登记，{count} 个本次导入的会话未能登记：{reason}（若原因是 cwd 在本机不存在，请在导入向导的路径映射里把源机路径映射到本机路径，或事后用离线修复工具 sessions repair）',
  'adapter.workspaceSessionsOutsideBundle': '另有 {count} 个会话不在此备份包内（本次未导入其数据），未登记；目标机本来就有这些会话则不受影响',
  'adapter.workspaceSessionsManual': '本机工作区服务不支持会话登记，{count} 个会话未登记：启动 DSH 后可在设置里重新导入，或用离线修复工具 sessions repair',
  'adapter.workspaceSessionsOwnerMissing': '找不到对应的目标工作区记录，{count} 个会话未登记：请确认备份里包含该工作区（导出会话时会自动连带其工作区）',
  ...
  'sync.credentialsReadFailed': '读取凭据文件失败，本次快照不含任何凭据值: {reason}',
  'sync.credentialsEmpty': '本机凭据文件为空或无法解析，本次快照不含任何凭据值',
  'sync.credentialsItemDesc': '凭据 {ref} 随加密快照迁移（确认后写入本机凭据存储）',
  'sync.credentialsNeedPassword': '快照 {id} 含加密凭据载荷，需要解密密码才能读取',
  ...
  'sync.sessionDeletionsRecorded': '已记录 {count} 个本机删除的历史对话（墓碑随快照传播，旧快照不会把它们带回来）',
  'sync.sessionsTombstoned': '已按远端墓碑跳过 {count} 个已删除的历史对话（不复活）',
  ...
  // ---------- 会话归位（issue #45：跨机恢复后按 cwd 归位到正确 projectKey） ----------
} as const;
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'export.sessionsWorkspacesCoupled': 'Also exported {count} workspace record(s) that own the selected sessions (a session only shows up when a workspace points at its cwd)',
  'export.sessionsDeclaredInWorkspaces': 'Declared the {count} exported session(s) in {workspaces} workspace record(s) (DSH workspace sessionIds coverage is very low; without this step those conversations stay invisible on the target machine)',
  'export.sessionsWorkspacesCarriedAll': 'None of the selected sessions is registered in any workspace record: all {count} workspace record(s) were exported as well (without a workspace those sessions stay invisible on the target machine)',
  'export.sessionsWithoutWorkspaces': 'This machine has no workspace record at all, so the sessions were exported on their own: they may not appear under any workspace on the target machine (a session only shows up when a workspace points at its cwd)',
  'export.sessionsWorkspacesUnreadable': 'Could not read the workspace records on this machine, so the owning workspaces are unknown: the sessions were exported on their own and may not show up on the target machine',
  ...
  'import.sessionsWithoutWorkspaces': 'This backup contains sessions but no workspace record at all: after import those conversations may not appear in any DSH workspace (re-export on the source machine with the current plugin build, which carries the owning workspaces automatically)',
  'import.subagentSessionsWithoutParents': 'This backup carries {count} subagent sessions whose parent conversation is not in the bundle ({units}): DSH only shows subagent sessions underneath their parent, so these will not appear in the workspace list after import (re-export on the source machine with the parent conversations selected)',
  'export.sessionParentsCoupled': 'Carried {count} parent conversation(s) along (DSH only shows subagent sessions underneath their parent; without the parent they stay invisible in the workspace after import)',
  'export.sessionParentsUncoupled': '{count} subagent session(s) could not carry their parent conversation: {detail} -- the imported child alone never shows up in a DSH workspace',
  'export.sessionChildrenCoupled': 'Carried {count} subagent conversation(s) along (DSH renders child conversations underneath their parent; the parent alone arrives with an empty tree)',
  'export.sessionRelationNotFound': 'related conversation not found on this machine',
  'export.sessionParentTooLarge': 'related conversation exceeds this section size limit',
  ...
  'import.secretAlreadyConfigured': 'Credential {ref} is already configured on this machine; re-entry skipped (local value kept)',
  'import.secretFromArchive': 'Credential {ref} is restored from the encrypted backup (written back on import)',
  ...
  'import.sessionRelocated': 'Session {unit} sat in a directory that does not match its first-frame cwd; relocated automatically: {from} -> {to} (directory only, content untouched)',
  'import.sessionRelocateFailed': 'Session {unit} sits in a directory that does not match its first-frame cwd and was not relocated ({reason}); if left as is, DSH will next fail to start with a corrupt session log -- provide the path mapping in the import wizard (or repair it later with the offline tool: sessions repair)',
  'import.sessionRewritten': 'Session {unit}: first-frame cwd rewritten by the path mapping {from} -> {to}, and the session directory relocated to {key}',
  'import.sessionRewriteFailed': 'Session {unit}: first-frame cwd rewrite failed ({reason}); session files and directory left untouched',
  'import.sessionRewriteRolledBack': 'Session {unit}: the first frame was rewritten but the directory could not be relocated ({reason}); the first frame was rolled back to its original value (avoids a corrupt session log on the next start)',
  'import.sessionRewriteRollbackFailed': 'Session {unit}: first frame rewritten, directory not relocated ({reason}), and the rollback FAILED -- inspect that session directory manually (DSH will otherwise fail to start with a corrupt session log)',
  ...
  'runs.cancelUnsupported': 'This task cannot be terminated manually (currently only an import supports cooperative cancellation at plan-item boundaries)',
  'runs.cancelWaitingSafePoint': 'Termination requested: it pauses after the current plan item (any subprocess of that item was aborted too)',
  'import.cancelRequested': 'Termination requested; pausing at the next safe point (plan-item boundary)...',
  'import.cancelAwaitingDecision': 'Paused: waiting for you to choose "roll back" or "keep applied items"',
  'import.cancelDecisionTimeout': 'Timed out waiting for the choice; handled with the safe-side default (roll back)',
  'import.cancelRollingBack': 'Rolled back this import as you chose ({count} applied items)',
  'import.cancelKept': 'Import terminated with applied items kept: {applied} applied, {skipped} not executed',
  'import.cancelKeptSnapshotKept': 'This import did not finish: the pre-import snapshot {snapshotId} is still kept, so you can roll it back later from the Backups page',
  'import.cancelKeptSkippedItem': 'Import was terminated by the user (applied items kept): this item was not executed',
  'import.cancelKeptJournalFailed': 'Failed to record that {id} was not executed ({reason}): the journal may disagree with the disk; trust the disk',
  'import.cancelKeptUnchecked': 'The launch safety audit skipped some checks ({items}): those aspects are unverified',
  'import.cancelKeptNoAudit': 'Launch safety audit did not run ({reason}): the boot safety of the kept state is NOT verified; if DSH fails to start, use incident recovery / the offline CLI',
  'bootSafety.bundlesPruned': '{count} bundle(s) in the plugin list cannot be resolved (not installed or removed): pruned {names} from dsh.profile.bundles -- leaving them in would make the next DSH start fail outright',
  'bootSafety.bundlesPrunedFailed': '{count} bundle(s) in the plugin list cannot be resolved ({names}), but writing the profile package.json back failed: {reason} -- prune them manually before starting DSH',
  'bootSafety.bundleListUnreadable': 'Cannot read the profile plugin list {rel} ({reason}): the startup-safe plugin set cannot be confirmed',
  'bootSafety.criticalFileUnreadable': 'Failed to read the boot-critical file {rel}: {reason}',
  'bootSafety.criticalFileUnparsable': 'Boot-critical file cannot be parsed ({rel}): {reason}',
  'bootSafety.patchFileMissing': 'pnpm-workspace.yaml patchedDependencies points at a patch file that does not exist: {ref} (a missing one makes pnpm reject every add)',
  'bootSafety.extraCheckFailed': 'Extra launch check failed: {reason}',
  ...
  'host.workspaceMissingTarget': 'Workspace {id} does not exist on this machine (cannot attach the session)',
  'host.fsPathNotAbsolute': 'Refusing to create a directory from a relative path (it would follow the process cwd): {path}',
  'host.fsPathDotDot': 'Refusing to create a directory from a path containing "..": {path}',
  'host.fsPathNotDirectory': 'A file (not a directory) already exists on this path: {path}',
  ...
  'adapter.fileSectionByteLimit': '{type} section reached the per-section byte limit {limit} ({kept} kept): {count} unit(s) skipped whole and NOT in this backup (units are never split) — {detail}',
  ...
  'adapter.workspaceDirCreated': 'Created workspace directories the backup declares but this machine lacked: {dirs}',
  'adapter.workspaceDirFailed': 'Failed to create the directory (falling back to the non-fatal warning): {path} - {msg}',
  'adapter.workspaceDirUnsafe': 'Path is not a safely creatable absolute path (relative or contains ..); no directory created: {path}',
  'adapter.workspaceSessionsAttached': 'Attached {count} session(s) to this workspace',
  'adapter.workspaceSessionsPending': '{ok} session(s) attached, {count} session(s) from this backup could not be attached: {reason} (if the cause is a cwd that does not exist on this machine, map the source-machine paths in the import wizard, or repair later with the offline tool: sessions repair)',
  'adapter.workspaceSessionsOutsideBundle': 'A further {count} session(s) are not part of this backup (their data was not imported) and stay unregistered; sessions that already exist on this machine are unaffected',
  'adapter.workspaceSessionsManual': 'The host workspace service cannot attach sessions; {count} session(s) are unregistered: re-import from Settings once DSH is up, or use the offline tool: sessions repair',
  'adapter.workspaceSessionsOwnerMissing': 'No matching target workspace record, {count} session(s) unregistered: make sure the backup contains that workspace (exporting sessions now carries their workspaces automatically)',
  ...
  'sync.credentialsReadFailed': 'Failed to read the credentials file; this snapshot carries no credential value: {reason}',
  'sync.credentialsEmpty': 'The local credentials file is empty or unparseable; this snapshot carries no credential value',
  'sync.credentialsItemDesc': 'Credential {ref} travels with the encrypted snapshot (written to the local credential store once confirmed)',
  'sync.credentialsNeedPassword': 'Snapshot {id} carries an encrypted credentials payload; a decryption password is required',
  ...
  'sync.sessionDeletionsRecorded': 'Recorded {count} locally deleted conversation(s) as tombstones (they travel with snapshots, so older snapshots will not bring them back)',
  'sync.sessionsTombstoned': 'Skipped {count} conversation(s) deleted on the remote side according to tombstones (no resurrection)',
  ...
  // ---------- session grouping (issue #45: relocate restored sessions by cwd) ----------
};
